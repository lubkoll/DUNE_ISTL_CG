...
namespace
{
  ...
  struct TestRCGSolver_2d : ::testing::Test
  {
    TestRCGSolver_2d()
      : A(), P(), sp(),
        cg( A, P, sp )
    ...
    ScalarProduct sp;
    Dune::RCGSolver< Vector, Vector , Dune::KrylovTerminationCriterion::ResidualBased > cg;
  };
...
TEST_F(TestRCGSolver_2d,NoStep)
{
  cg.setMaxSteps(0);
  auto x = initialGuess();
...
TEST_F(TestRCGSolver_2d,OneStep)
{
  cg.setMaxSteps(1);
  auto x = initialGuess();
  auto b = rightHandSide();
...
TEST_F(TestRCGSolver_2d,TwoSteps)
{
  cg.setMaxSteps(2);
  auto x = initialGuess();
  auto b = rightHandSide();
