...
namespace
{
  ...
  struct TestCGSolver_2d : ::testing::Test
  {
    TestCGSolver_2d()
      : A(), P(), sp(),
        cg( A, P, sp )
    ...
    ScalarProduct sp;
    Dune::MyCGSolver< Vector, Vector , Dune::KrylovTerminationCriterion::ResidualBased > cg;
  };
...
TEST_F(TestCGSolver_2d,OneStep)
{
  cg.setMaxSteps(1);
  auto x = initialGuess();
  auto b = rightHandSide();
...
TEST_F(TestCGSolver_2d,TwoSteps)
{
  cg.setMaxSteps(2);
  auto x = initialGuess();
  auto b = rightHandSide();
